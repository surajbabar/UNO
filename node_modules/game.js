...
game.catchUno = function (caughtPlayerIndex, smartPlayer) {
    var caughtPlayer = game.players[caughtPlayerIndex];
    if (player.checkUno(caughtPlayer)) {
        player.takeCard(caughtPlayer, draw());
        player.takeCard(caughtPlayer, draw());
        log.push(caughtPlayer.name + " caught by " + smartPlayer.name + "\n\t\t"+getTime());
    }
    else
        log.push("catch was not valid on " + caughtPlayer.name +" \n\t\t"+getTime());
}
...
game.drawTwoCards = function (plyr) {
    ...
    updateLogAfterDrawTwo(plyr);
    drawTwoRun = 0;
    hint = "play a +2 or " + runningColor + " card";
    nextTurn();
}

game.moveForwardAsPlayerTookNoActionOnDrawnCard = function (plyr) {
    log.push("no action played by " + plyr.name + " after draw a card\n\t\t"+getTime());
    nextTurn();
}
...
var updateLogAfterDrawTwo = function (plyr) {
    log.push(plyr.name + " drew " + drawTwoRun * 2 + " " + " cards" + "\n\t\t"+getTime());
}

var updateLogAfterPlay = function (plyr, playedCard) {
    var cardDetail = playedCard.color + " " + playedCard.sign;

    cardDetail =playedCard.color == "black" ? playedCard.sign + " and selected " + runningColor + " color":cardDetail;
    log.push(plyr.name + " played a " +cardDetail+ " \n\t\t"+getTime());
}

var updateLogAfterInitialize = function (playedCard) {
    log.push(" Game starts with " + playedCard.color + " " + playedCard.sign + "\n\t\t"+getTime());
}
